...
def verification_record(subject=IDENTITY, **extra):
    value = {"reference": "verification:unit", "subject": subject, **verification_request(), "status": "PASS"}
    value.update(extra)
    return value


def decision(key="task-one", identity=IDENTITY):
    return {"kind": "decision", "task_key": key, "reference": "decision:task-one", "subject": identity}
...
def handoff(key="task-one", worker_reference="writer-one", identity=IDENTITY, **extra):
    value = {
        ...
        "verification_references": [{"task_key": key, "reference": "verification:unit", "identity": identity}],
...
def completed_candidate(workflow, value, key="task-one"):
    ...
    workflow.record_verification(value, verification_record())
