...
def completed_candidate(workflow, value, key="task-one"):
    ...
    workflow.record_verification(value, {"task_key": key, "reference": "verification:gate", "subject": IDENTITY, "command": ["python", "scripts/temper-gate.py", "all"], "scope": ["repository"], "environment": {"lock": "committed"}, "side_effects": [], "status": "PASS", "final_verifier": "registered-final-verifier"})
...
def test_integration_requires_cold_review_final_gate_and_public_safety_evidence():
    ...
    with pytest.raises(workflow.WorkflowError, match="final-verifier"):
        workflow.authorize(value, request)
    workflow.record_verification(value, {"task_key": "task-one", "reference": "verification:gate-after-review", "subject": IDENTITY, "command": ["python", "scripts/temper-gate.py", "all"], "scope": ["repository"], "environment": {"lock": "committed"}, "side_effects": [], "status": "PASS", "final_verifier": "registered-final-verifier"})
...
def test_guarded_second_writer_is_reachable_and_third_or_unguarded_writer_is_rejected():
    ...
    assert workflow.register_worker(value, {"reference": "writer-two", "task_key": "task-two", "independence_guards": guards})["status"] == "ACTIVE"
    with pytest.raises(workflow.WorkflowError, match="third"):
        workflow.register_worker(value, {"reference": "writer-three", "task_key": "task-three", "independence_guards": guards})
...
def test_duplicate_task_attempt_and_ambiguous_spawn_reconciliation_are_rejected_or_durable():
    ...
    workflow.acquire_ownership(value, {"task_key": "task-one", "paths": ["scripts/example.py"]})
    workflow.advance(value, {"task_key": "task-one"})
    value["workers"].append({"reference": "duplicate", "task_key": "task-one", "task_class": "normal_implementation", "writer": True, "status": "ACTIVE", "route": task()["route"], "attempt": 1, "monitor": workflow._new_monitor("ACTIVE")})
    with pytest.raises(workflow.WorkflowError, match="task attempt"):
        workflow.validate_state(value)
    value["workers"].pop()
