...
def resolve_uv(bootstrap_mode: str) -> Iterator[tuple[list[str], dict[str, str] | None]]:
    if shutil.which("uv"):
        yield ["uv"], None
        return
...
def run_uv(command: Sequence[str], uv_command: Sequence[str], uv_env: dict[str, str] | None) -> int:
    return run_command([*uv_command, *command], env=uv_env)


def run_setup(uv_command: Sequence[str], uv_env: dict[str, str] | None) -> int:
    return run_uv(["sync", "--dev"], uv_command, uv_env)
...
def run_direct_python_checks() -> int:
    for command in (
